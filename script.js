...
function initNetwork(canvasId, nodeCount=50){
  ...
  for(let i=0;i<nodeCount;i++){
    nodes.push({ x:Math.random()*nc.width, y:Math.random()*nc.height, vx:(Math.random()-0.5)*0.5, vy:(Math.random()-0.5)*0.5, r:Math.random()*2.5+0.8, a:Math.random()*0.7+0.2 });
  }
  let mouse = {x:-999, y:-999};
  nc.addEventListener('mousemove', e=>{ const rect=nc.getBoundingClientRect(); mouse.x=e.clientX-rect.left; mouse.y=e.clientY-rect.top; });
  window.addNetNode = (x,y)=> nodes.push({x,y,vx:(Math.random()-0.5)*0.5,vy:(Math.random()-0.5)*0.5,r:3,a:1});
  (function anim(){
